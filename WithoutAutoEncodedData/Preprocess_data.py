...
def train_graph():
    ...
    print("before cna")
    df1 = pandas.DataFrame(X_train_cna)
    df2 = pandas.DataFrame(X_test_cna)
    df = pandas.concat([df1, df2], axis=0)
    df.to_csv('autoEncoderData/cna_data.csv', index=False, header=False)
    print("after cna")

    print("before rna")
    df1 = pandas.DataFrame(X_train_rna)
    df2 = pandas.DataFrame(X_test_rna)
    df = pandas.concat([df1, df2], axis=0)
    df.to_csv('autoEncoderData/rna_data.csv', index=False, header=False)
    print("after rna")

    print("before gene")
    df1 = pandas.DataFrame(X_train_gene)
    df2 = pandas.DataFrame(X_test_gene)
    df = pandas.concat([df1, df2], axis=0)
