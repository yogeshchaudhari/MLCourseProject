...
d_matrix, d_class = dnn_md2.load_txt(dnn_md2.D2,619) #619 features 
...
if os.path.exists(dnn_md2.Kfold):
    kf1 = pickle.load(open(dnn_md2.Kfold,"rb"))
    print("successfully loading already existing kfold index!")
else:
    kf1 = KFold(dnn_md2.K, True)
    pickle.dump(kf1, open(dnn_md2.Kfold, "wb"))
...
# validation
p_valid_all = dnn_md2.beta * p_valid_all2
auc_fcn, pr_auc_fcn = ut.calc_auc_t(cls_valid_all2[:, 1], p_valid_all[:, 1])

pre_f, rec_f, f1_f, acc_f = ut.get_precision_and_recall_f1(np.argmax(cls_valid_all2, 1), np.argmax(p_valid_all, 1))

print("DNN_CNA-validation## ACC: %s,AUC %s,PRE %s,REC %s,F1 %s, PR_AUC %s" % (acc_f, auc_fcn, pre_f, rec_f, f1_f, pr_auc_fcn))

# test
class_predict_fcn =  dnn_md2.beta * class_predict_fcn2
...
print("DNN_CNA-testing## ACC: %s,AUC %s,PRE %s,REC %s,F1 %s, PR_AUC %s" % (acc_f, auc_fcn, pre_f, rec_f, f1_f, pr_auc_fcn))

name = dnn_md2.name + '_' + str(dnn_md2.hidden_units[0]) + '-' + str(dnn_md2.hidden_units[1]) + '-' + str(
    dnn_md2.hidden_units[2]) + '-' + str(dnn_md2.hidden_units[3]) + '_' + str(dnn_md2.alpha) + '_' + str(dnn_md2.beta)
# SCORE

np.savetxt("results/result_METABRIC1/p_valid_all2.txt", p_valid_all2)


np.savetxt("results/result_METABRIC1/cls_valid_all1.txt", cls_valid_all2)
